...
class AppUserAuth:
    ...
    def _ensure_config(self) -> None:
        if not self._api_key() or not self._db_url():
            raise RuntimeError(
                "Firebase config missing (database_url/api_key). Set in firebase.config.json."
            )
    ...
    def get_bookmarks(self) -> list[dict]:
        ...
        if not auth:
            return []
        data = self._request("GET", f"app_users/{auth['localId']}/bookmarks", None, auth)
        if not data:
            return []
    ...
    def replace_bookmarks(self, bookmarks: list[dict]) -> None:
        ...
        self._request("PUT", f"app_users/{auth['localId']}/bookmarks", bookmarks, auth)
    ...
    def get_user_detail(self, uid: str) -> dict | None:
        auth = self.get_auth()
        if not auth:
            raise RuntimeError("Not logged in.")
        data = self._request("GET", f"app_users/{uid}", None, auth)
        return data if isinstance(data, dict) else None

    def log_admin_action(self, action: str, target_uid: str, target_username: str, meta: dict | None = None) -> None:
        auth = self.get_auth()
        if not auth:
            return
        admin_uid = auth.get("localId", "")
        if not admin_uid:
            return
        payload = {
            "action": action,
        ...
        try:
            self._request("POST", f"admin_logs/{admin_uid}", payload, auth)
        except Exception:
            pass

    def list_admin_logs(self, limit: int = 20) -> list[dict]:
        auth = self.get_auth()
        if not auth:
            raise RuntimeError("Not logged in.")
        admin_uid = auth.get("localId", "")
        if not admin_uid:
            return []
        data = self._request("GET", f"admin_logs/{admin_uid}", None, auth)
        if not isinstance(data, dict):
            return []
        items = []
