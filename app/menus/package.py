...
def _inspect_package_detail(package: dict, width: int) -> None:
    raw = json.dumps(package, indent=2, ensure_ascii=True)
    lines = raw.splitlines()
    print_paged(lines, "?? INSPECT DETAIL (RAW)", width=width, color=M, page_size=16)
...
def show_package_details(
    ...
    if detail and str(detail).strip():
        ...
        print_card(tnc_lines, "📜 SYARAT & KETENTUAN", width=width, color=M)
    ...
    while True:
        options_grid = [
            ...
            f"{C}[{W}I{C}] {W}Inspect detail (raw){RESET}",
        ]
        ...
        if choice == "00":
            ...
            _inspect_package_detail(package, width)
            continue
