...
_CONFIG = load_config()
BASE_CIAM_URL = str(_CONFIG.get("base_ciam_url", "")).strip()
if not BASE_CIAM_URL:
    raise ValueError("base_ciam_url missing in firebase.config.json")

BASIC_AUTH = str(_CONFIG.get("basic_auth", "")).strip()
if not BASIC_AUTH:
    raise ValueError("basic_auth missing in firebase.config.json")
...
AX_FP = str(_CONFIG.get("ax_fp", "")).strip() or load_ax_fp()
