...
_CONFIG = load_config()
API_KEY = str(_CONFIG.get("x_api_key", "")).strip()
AES_KEY_ASCII = str(_CONFIG.get("aes_key_ascii", "")).strip()
AX_FP_KEY = str(_CONFIG.get("ax_fp_key", "")).strip()
AX_FP = str(_CONFIG.get("ax_fp", "")).strip()

if not API_KEY:
    raise ValueError("x_api_key missing in firebase.config.json")
if not AES_KEY_ASCII:
    raise ValueError("aes_key_ascii missing in firebase.config.json")
...
def load_ax_fp() -> str:
    if not AX_FP_KEY:
        raise ValueError("ax_fp_key missing in firebase.config.json")
    fp_path = "ax.fp"
...
def ax_device_id() -> str:
    android_id = AX_FP or load_ax_fp()  # Use configured fingerprint when available
